...
import { GLPRoduct } from '~/components copy/GLProduct';
...
export default function ProductHandle() {
  ...
  const [m, setM] = useAtom(model);
  const [localProduct, setLocalProduct]: any = useState({
    name: "",
    index: "",
    url: ""
  })
  ...
  const setCoords = () => {
    const { width, height, left, top } = productRef?.current.getBoundingClientRect()
  ...
  useEffect(() => {
    setCoords()
    stableProductData && console.log(m)
    ...
    m.map((item: any, index: number) => {
      if (item.name === stableProductData.handle) {
        setLocalProduct({
          name: item.name,
          index: item.index,
          url: item.url
  ...
  const node: any = JSON.parse(stableProductData.metafield.value)

  return (
    <>
      <section>
        <div className="product-grid w-full max-w-full grid items-start gap-6 lg:gap-10 md:grid-cols-[repeat(2,_minmax(auto,_1fr))]">
          <div className=" product min-w-0 h-full w-full max-w-full grid md:grid-flow-row md:p-0 md:grid-cols-2 md:w-full lg:col-span-2">
            <div ref={productRef} className="h-full md:col-span-2 min-w-0 min-h-60 snap-center card-image md:w-full w-full">
              <div
          ...
          <div className='grid gap-2 desc border-t border-[#222] md:pt-6 backdrop-blur-md p-2 rounded-sm text-indigo-100'>
